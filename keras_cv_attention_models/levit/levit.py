...
def LeViT(
    ...
    if num_classes == 0:
        ...
        out_z = batchnorm_with_activation(nn, activation=None, name="head_")
        out_z = layers.Flatten()(out_z)
        # out_cl = layers.Dense(32, dtype="float32", activation="gelu", name="predictions1")(out_z)
        # out = layers.Dense(4, dtype="float32", activation="softmax", name="predictions")(out_z)
        
        # out_label = layers.Dense(32, dtype="float32", activation="gelu", name="predictions_label1")(out_z)
        # out_label = layers.Dense(1, dtype="float32", activation=classifier_activation, name="predictions_label")(out_label)
        # out = [out_cl, out_label]
        out = layers.Dense(num_classes, dtype="float32", activation=classifier_activation, name="predictions")(out_z)
