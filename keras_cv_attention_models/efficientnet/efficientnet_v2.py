...
#     tf.keras.layers.RandomRotation(factor=1, fill_mode='constant', interpolation='constant', fill_value=0),
...
def EfficientNetV2(
    ...
    # inputs = tf.image.per_image_standardization(inputs) * 0.23 + 0.49
    # inputs = tf.image.per_image_standardization(inputs[:, :, :, 0:2]) * 0.23 + 0.49
    # inputs2 = tf.image.per_image_standardization(tf.expand_dims(inputs[:, :, :, 2], axis=3)) * 0.23 + 0.49
    # inputs = tf.keras.layers.Concatenate(axis=3)([inputs1, inputs2])
    ...
    nn1 = conv2d_no_bias(nn1, stem_width, 3, strides=first_strides, padding="same", use_torch_padding=is_torch_mode, name="stem_1")
    nn1 = batchnorm_with_activation(nn1, activation=activation, epsilon=bn_eps, name="stem_1")

    # nn2 = conv2d_no_bias(nn2, stem_width, 3, strides=first_strides, padding="same", use_torch_padding=is_torch_mode, name="stem_2")
    # nn2 = batchnorm_with_activation(nn2, activation=activation, epsilon=bn_eps, name="stem_2")
    ...
    for id, (expand, out_channel, depth, stride, se_ratio, kernel_size) in enumerate(zip(expands, out_channels, depthes, strides, se_ratios, kernel_sizes)):
        ...
        for block_id in range(depth):
            name1 = "stack_{}_block{}_1".format(id, block_id)
            name2 = "stack_{}_block{}_2".format(id, block_id)
            stride = stride if block_id == 0 else 1
            shortcut = cur_use_shortcuts if out == pre_out and stride == 1 else False
            block_drop_rate = drop_connect_rate * global_block_id / total_blocks
            nn1 = inverted_residual_block(
                nn1, out, stride, expand, shortcut, kernel_size, block_drop_rate, se_ratio, cur_is_fused, **blocks_kwargs, activation=activation, name=name1
            )
            # nn2 = inverted_residual_block(
            #     nn2, out, stride, expand, shortcut, kernel_size, block_drop_rate, se_ratio, cur_is_fused, **blocks_kwargs, activation=activation, name=name2
            # )            
            pre_out = out
            global_block_id += 1
    ...
    if output_conv_filter > 0:
        ...
        nn1 = conv2d_no_bias(nn1, output_conv_filter, 1, strides=1, padding="valid", use_torch_padding=is_torch_mode, name="post_1")
        nn1 = batchnorm_with_activation(nn1, activation=activation, epsilon=bn_eps, name="post_1")
        # nn2 = conv2d_no_bias(nn2, output_conv_filter, 1, strides=1, padding="valid", use_torch_padding=is_torch_mode, name="post_2")
        # nn2 = batchnorm_with_activation(nn2, activation=activation, epsilon=bn_eps, name="post_2")        
        
    
    nn_label = output_block(nn1, num_classes=num_classes, drop_rate=dropout, classifier_activation=classifier_activation)
    
    model = models.Model(inputs=inputs, outputs=nn_label, name=model_name)
