...
def EfficientViT_B(
    ...
    # nn = conv2d_no_bias(inputs[:, :, :, 0:2], stem_width // 8, 3, strides=2, padding="same", name="stem_1_")
    ...
    # nn_r = conv2d_no_bias(tf.expand_dims(inputs[:, :, :, 2], axis=3), stem_width // 8, 3, strides=2, padding="same", name="stem_1_radon")
    # nn_r = batchnorm_with_activation(nn, activation=activation, name="stem_1_radon")
    # nn_r = conv2d_no_bias(nn, stem_width // 4, 3, strides=2, padding="same", name="stem_2_radon")
    # nn_r = batchnorm_with_activation(nn, activation=activation, name="stem_2_radon")
    # nn_r = conv2d_no_bias(nn, stem_width // 2, 3, strides=2, padding="same", name="stem_3_radon")
    # nn_r = batchnorm_with_activation(nn, activation=activation, name="stem_3_radon")
    # nn_r = conv2d_no_bias(nn, stem_width, 3, strides=2, padding="same", name="stem_4_radon")
    # nn_r = batchnorm_with_activation(nn, activation=None, name="stem_4_radon")
    # nn_r = mb_conv(nn, stem_width, shortcut=True, expansion=1, is_fused=is_fused[0], activation=activation, name="stem_MB_radon")


    # nn_r = conv2d_no_bias(tf.expand_dims(inputs[:, :, :, 2], axis=3), stem_width, 3, strides=2, padding="same", name="stem_radon")
    # nn_r = batchnorm_with_activation(nn, activation=activation, epsilon=BATCH_NORM_EPSILON, name="stem_")
    # nn_r = mb_conv(nn, stem_width, shortcut=True, expansion=1, is_fused=is_fused[0], activation=activation, name="stem_MB_radon")

    # nn_f = tf.keras.layers.Concatenate(axis=3)([nn, nn_r])
    ...
    for stack_id, (num_block, out_channel, block_type) in enumerate(zip(num_blocks, out_channels, block_types)):
        ...
        for block_id in range(num_block):
            ...
            # name_r = "stack_{}_block_{}_radon".format(stack_id + 1, block_id + 1)
            # name_f = "stack_{}_block_{}_fuse".format(stack_id + 1, block_id + 1)
            ...
            if is_conv_block or block_id == 0:
                ...
                # cur_name_r = (name_r + "downsample_radon") if stride > 1 else name_r
                # cur_name_f = (name_r + "downsample_fuse") if stride > 1 else name_f
                ...
                # nn_r = mb_conv(nn_r, out_channel, shortcut, stride, cur_expansion, cur_is_fused, use_bias, use_norm, block_drop_rate, activation, name=cur_name_r)
                # nn_f = mb_conv(nn_f, out_channel, shortcut, stride, cur_expansion, cur_is_fused, use_bias, use_norm, block_drop_rate, activation, name=cur_name_f)
            else:
                ...
                # attn_r = lite_mhsa(nn_r, num_heads=num_heads, key_dim=head_dimension, sr_ratio=5, name=name_r + "attn_")
                # attn_f = lite_mhsa(nn_f, num_heads=num_heads, key_dim=head_dimension, sr_ratio=5, name=name_f + "attn_")
                ...
                # nn_r = mb_conv(nn_r, out_channel, shortcut, stride, cur_expansion, cur_is_fused, use_bias, use_norm, block_drop_rate, activation, name=name_r)
                # nn_f = mb_conv(nn_f, out_channel, shortcut, stride, cur_expansion, cur_is_fused, use_bias, use_norm, block_drop_rate, activation, name=name_f)
            global_block_id += 1
    ...
    if output_filters[0] > 0:
        ...
        # nn_r = conv2d_no_bias(nn_r, output_filters[0], name="features_radon")
        # nn_r = batchnorm_with_activation(nn_r, activation=activation, epsilon=BATCH_NORM_EPSILON, name="features_radon")        
        # nn_f = conv2d_no_bias(nn_f, output_filters[0], name="features_fuse")
        # nn_f = batchnorm_with_activation(nn_f, activation=activation, epsilon=BATCH_NORM_EPSILON, name="features_fuse")    
    if num_classes > 0:
        nn = layers.GlobalAveragePooling2D(name="avg_pool")(nn) if len(nn.shape) == 4 else nn
        # nn_r = layers.GlobalAveragePooling2D(name="avg_pool_radon")(nn_r) if len(nn_r.shape) == 4 else nn_r
        # nn_f = layers.GlobalAveragePooling2D(name="avg_pool_fuse")(nn_f) if len(nn_f.shape) == 4 else nn_f
        # nn = tf.keras.layers.Concatenate(axis=1)([nn, nn_r])

        if dropout > 0:
            nn = layers.Dropout(dropout, name="head_drop")(nn)
            # nn_r = layers.Dropout(dropout, name="head_drop_radon")(nn_r)
            # nn_f = layers.Dropout(dropout, name="head_drop_fuse")(nn_f)
        if output_filters[1] > 0:
            ...
            nn_m = layers.Dense(output_filters[1], use_bias=False, name="pre_predictions_m")(nn)
            nn_m = layer_norm(nn, epsilon=LAYER_NORM_EPSILON, name="pre_predictions_m")
            nn_m = activation_by_name(nn, activation=activation, name="pre_predictions_m")

            # nn_r = layers.Dense(output_filters[1], use_bias=False, name="pre_predictions_radon")(nn_r)
            # nn_r = layer_norm(nn_r, epsilon=LAYER_NORM_EPSILON, name="pre_predictions_radon")
            # nn_r = activation_by_name(nn_r, activation=activation, name="pre_predictions_radon")
            
            # nn_f = layers.Dense(output_filters[1], use_bias=False, name="pre_predictions_fuse")(nn_f)
            # nn_f = layer_norm(nn_f, epsilon=LAYER_NORM_EPSILON, name="pre_predictions_fuse")
            # nn_f = activation_by_name(nn_f, activation=activation, name="pre_predictions_fuse")

        # nn = layers.Dense(num_classes, dtype="float32", activation=None, name="predictions")(nn)
        # nn_r = layers.Dense(num_classes, dtype="float32", activation=None, name="predictions")(nn_r)
        # nn_m = layers.Dense(num_classes, dtype="float32", activation=None, name="predictions_m")(nn_m)

        # nn_r = layers.Dense(num_classes, dtype="float32", activation=None, name="predictions_radon")(nn_r)
        # nn_f = layers.Dense(num_classes, dtype="float32", activation=None, name="predictions_fuse")(nn_f)

        # nn = nn * nn_r #* nn_f
        # nn = layers.Dense(num_classes, dtype="float32", activation=classifier_activation, name="predictions_final")(nn)
        # nn_m = layers.Dense(num_classes, dtype="float32", activation="sigmoid", name="predictions_ranking")(nn_m)
    model = models.Model(inputs=inputs, outputs=[nn, nn_m], name=model_name)
