...
def output_block(inputs, filters=0, activation="relu", num_classes=1000, drop_rate=0, classifier_activation="softmax", is_torch_mode=True, act_first=False):
    ...
    if num_classes > 0:
        ...
        # nn = layers.Dense(128, dtype="float32", activation=None, name="latent")(nn)    
        # nn_class = layers.Dense(num_classes, dtype="float32", activation=classifier_activation, name="predictions")(nn)
        nn_label = layers.Dense(4, dtype="float32", activation="softmax", name="predictions_label")(nn)
    return nn_label#, nn_label#, nn #, nn_param

# def output_block(inputs1, inputs2, filters=0, activation="relu", num_classes=1000, drop_rate=0, classifier_activation="softmax", is_torch_mode=True, act_first=False):
#     nn1 = inputs1
#     nn2 = inputs2
#     layer_normal = layers.LayerNormalization(axis=1)
#     if filters > 0:  # efficientnet like
#         bn_eps = BATCH_NORM_EPSILON if is_torch_mode else TF_BATCH_NORM_EPSILON
#         nn = conv2d_no_bias(nn, filters, 1, strides=1, use_bias=act_first, use_torch_padding=is_torch_mode, name="features_")  # Also use_bias for act_first
#         nn = batchnorm_with_activation(nn, activation=activation, act_first=act_first, epsilon=bn_eps, name="features_")
...
#     if num_classes > 0:
#         nn1 = layers.GlobalAveragePooling2D(name="avg_pool1")(nn1) if len(nn1.shape) == 4 else nn1
#         nn2 = layers.GlobalAveragePooling2D(name="avg_pool2")(nn2) if len(nn2.shape) == 4 else nn2
        
#         nn = layers.Concatenate(axis=1)([layer_normal(nn1), layer_normal(nn2)])
#         nn = layers.Dense(2048, dtype="float32", activation='relu', name="fuse1")(nn)
#         nn = layers.Dense(1024, dtype="float32", activation='relu', name="fuse2")(nn)
#         nn = layers.Dense(512, dtype="float32", activation='relu', name="fuse3")(nn)
#         nn = layers.Dense(128, dtype="float32", activation='relu', name="fuse4")(nn)

#         if drop_rate > 0:
#             nn1 = layers.Dropout(drop_rate, name="head_drop1")(nn1)
#             nn2 = layers.Dropout(drop_rate, name="head_drop2")(nn2)
#             nn = layers.Dropout(drop_rate, name="head_drop")(nn)

#         nn1 = layers.Dense(num_classes, dtype="float32", activation=classifier_activation, name="predictions1")(nn1)
#         nn2 = layers.Dense(num_classes, dtype="float32", activation=classifier_activation, name="predictions2")(nn2)
