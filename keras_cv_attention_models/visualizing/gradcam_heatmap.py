...
size = 224
def make_and_apply_gradcam_heatmap(model, image, img_vis, layer_name="auto", rescale_mode="auto", pred_index=None, alpha=0.95, use_v2=True, plot=True):
    ...
    image = np.array(image).reshape(size, size, 1)
    img_vis = np.array(img_vis).reshape(size, size, 1)
    ...
    superimposed_img = apply_heatmap(img_vis, heatmap, alpha=alpha, plot=plot)
