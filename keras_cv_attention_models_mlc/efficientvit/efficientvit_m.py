...
def EfficientViT_M(
    ...
    radon_inputs = radon(inputs, theta=theta)
    ...
    # Radon Inputs
    nn_r = conv2d_no_bias(radon_inputs, stem_width // 8, 3, strides=2, padding="same", name="stem_1_")
    nn_r = batchnorm_with_activation(nn, activation=activation, name="stem_1_")
    nn_r = conv2d_no_bias(nn, stem_width // 4, 3, strides=2, padding="same", name="stem_2_")
    nn_r = batchnorm_with_activation(nn, activation=activation, name="stem_2_")
    nn_r = conv2d_no_bias(nn, stem_width // 2, 3, strides=2, padding="same", name="stem_3_")
    nn_r = batchnorm_with_activation(nn, activation=activation, name="stem_3_")
    nn_r = conv2d_no_bias(nn, stem_width, 3, strides=2, padding="same", name="stem_4_")
    nn_r = batchnorm_with_activation(nn, activation=None, name="stem_4_")
